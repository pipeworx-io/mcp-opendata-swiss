...
`tools/list` at `https://gateway.pipeworx.io/opendata-swiss/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/opendata_swiss_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"energy"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/opendata_swiss_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "opendata-swiss": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-opendata-swiss"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-opendata-swiss
```
